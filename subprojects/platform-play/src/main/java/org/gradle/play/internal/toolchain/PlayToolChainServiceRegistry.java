...
public class PlayToolChainServiceRegistry extends AbstractPluginServiceRegistry {
    ...
    private static class ProjectScopeCompileServices {
        PlayToolChainInternal createPlayToolChain(JavaForkOptionsFactory forkOptionsFactory, ClassPathRegistry classPathRegistry, WorkerDaemonFactory workerDaemonFactory, ConfigurationContainer configurationContainer, DependencyHandler dependencyHandler, WorkerProcessFactory workerProcessBuilderFactory, WorkerDirectoryProvider workerDirectoryProvider, ClasspathFingerprinter fingerprinter) {
            return new DefaultPlayToolChain(forkOptionsFactory, classPathRegistry, workerDaemonFactory, configurationContainer, dependencyHandler, workerProcessBuilderFactory, workerDirectoryProvider, fingerprinter);
        }
    }
}
